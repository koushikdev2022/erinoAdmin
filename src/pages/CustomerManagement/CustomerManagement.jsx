...
const FlowbiteToggleSwitch = React.memo(
  ({ isActive, onToggle, isLoading, customerId }) => {
...
const CustomerManagement = () => {
  ...
  const dispatch = useDispatch();
  const { customerLists, loading, error, singleCustomer } = useSelector(
    (state) => state?.coustomers
  );
  ...
  const transformedRowData = useMemo(() => {
    return customerLists?.data
      ? customerLists.data.map((customer) => ({
          id: customer.id,
          name: customer.full_name,
          phone: customer.mobile,
          balance: `${customer.wallet?.balance || "0"}`,
          points: `${customer.promoCoins?.total_points || 0} Points`,
          pointsstatus: `${customer.promoCoins?.valid_points || 0}/${
            customer.promoCoins?.expired_points || 0
          }`,
          status: customer.status === 1 ? "Active" : "Inactive",
          lasttransaction: customer.last_transaction?.created_at
            ? new Date(customer.last_transaction.created_at).toLocaleDateString(
                "en-GB",
                {
                  day: "2-digit",
                  month: "short",
      ...
      <div className="wrapper_area my-0 mx-auto p-6 rounded-xl bg-white">
        <div className="h-full lg:h-screen">
          ...
          <div className="mb-4 flex justify-end gap-4 items-center">
            <div className="relative flex-1 max-w-md">
              <div className="absolute inset-y-0 left-10 z-10 pl-3 flex items-center pointer-events-none">
                <HiSearch className="h-5 w-5 text-gray-400" />
              </div>
              <TextInput
                type="text"
